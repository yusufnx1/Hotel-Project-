...
namespace HotelProject.WebUI.Controllers
{
    public class GuestController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var responsiveMessage = await client.GetAsync("http://localhost:5069/api/Guest");
        ...
        public async Task<IActionResult> AddGuest(CreateGuestDto createGuestDto)
        {
            ...
            var responsiveMessage = await client.PostAsync("http://localhost:5069/api/Guest", stringContent);
            if (responsiveMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responsiveMessage = await client.DeleteAsync($"http://localhost:5069/api/Guest/{id}");
            if (responsiveMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responsiveMessage = await client.GetAsync($"http://localhost:5069/api/Guest/{id}");
            if (responsiveMessage.IsSuccessStatusCode)
            {
                var jsonData = await responsiveMessage.Content.ReadAsStringAsync();
